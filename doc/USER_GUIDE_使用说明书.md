...
- **配置项**：
  ...
  - **显示指标**：
    - `自动（余额优先）`：有账户余额的厂商显示余额，否则显示 5 小时额度，再退回周期额度；
    - `5 小时剩余额度` / `周期剩余额度`：显示剩余百分比，并带上 `5时` / `周` 紧凑标记；
    - `账户余额`：仅显示金额。
- **展示效果**：例如 `百炼 5时 62%`、`DeepSeek ¥12.34`；数据未就绪或未授权时显示 `百炼 --`。鼠标悬停在图标上可看到完整说明（额度名称、剩余比例、重置倒计时）。
