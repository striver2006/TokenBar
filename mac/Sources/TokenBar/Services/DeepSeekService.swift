...
public final class DeepSeekService: @unchecked Sendable {
    ...
    public func fetchQuota(
        ...
        let balanceString = await fetchBalance(apiKey: cleanKey)
    ...
    private func fetchBalance(apiKey: String) async -> String? {
        ...
        guard let (data, resp) = try? await URLSession.shared.data(for: req),
              ...
              let total = first["total_balance"] as? String else {
            return nil
        }
        let currency = (first["currency"] as? String) ?? "CNY"
        return "\(currency == "CNY" ? "¥" : "$")\(total)"
    }
}
